...
% optimization: change the for loop with a sum(): 33% of the time spent
% here for large problems
function pn = calcpn(IX,IZ,dns,derivatives,field)
    pn=0; ix=IX+1; iz=dns.nz+1+IZ; k2=IZ*IZ/field.y(end)+(IX*dns.alfa0)^2; n=sum(derivatives.drd{iz}(end,:)~=0);
